...
function timeSpanToString(startDate, endDate) {
  ...
  const getMilliseconds = Math.floor(Math.abs(time % 1000));
  ...
  function numToStr(num, zeroCout) {
    return num.toString().padStart(zeroCout, 0);
  }

  return `${numToStr(getHours, 2)}:${numToStr(getMinutes, 2)}:${numToStr(getSeconds, 2)}.${numToStr(getMilliseconds, 3)}`;
}
